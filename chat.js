...
10. Place all explanations, summaries, instructions (e.g., "Want me to turn any of these into a powerful visual? Just upload your photo and type the row number..."), recommendations, and observations AFTER the table.
...
Concise Summary:`;
  ...
  const APP_CONFIG = {
    apiMode: 'n8n', 
    n8nWebhookUrl: import.meta.env.VITE_N8N_WEBHOOK_URL || '/n8n-webhook-test/6fe6482f-6741-4e81-87d0-95de7cb780c2',
  ...
  // Image Uploader Modal Elements
  const imageUploaderModal = $('imageUploaderModal');
  const uploaderBackdrop = $('uploaderBackdrop');
  const uploaderClose = $('uploaderClose');
  const uploaderFileInput = $('uploaderFileInput');
  const uploaderWebhookUrl = $('uploaderWebhookUrl');
  const uploaderPreview = $('uploaderPreview');
  const uploaderStatus = $('uploaderStatus');
  const uploaderSubmitBtn = $('uploaderSubmitBtn');
  const uploaderCancelBtn = $('uploaderCancelBtn');
  const webhookUploadBtn = $('webhookUploadBtn');
  const uploaderFieldName = $('uploaderFieldName');
  const uploaderSendAsJson = $('uploaderSendAsJson');
  const uploaderShowDebug = $('uploaderShowDebug');
  const uploaderSendBinary = $('uploaderSendBinary');
  const uploaderUseProxy = $('uploaderUseProxy');
  let pendingFiles = [];
  let busy = false;
  let chatInner = null;
  ...
  // Image Uploader Modal Functions
  let uploaderSelectedFile = null;

  function closeImageUploader() {
    if (!imageUploaderModal) return;
    imageUploaderModal.hidden = true;
    imageUploaderModal.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('uploader-open');
    uploaderSelectedFile = null;
    uploaderFileInput.value = '';
    uploaderPreview.innerHTML = '';
    uploaderStatus.innerHTML = '';
    uploaderStatus.className = 'uploader-status';
    releaseFocus(imageUploaderModal);
  }

  function openImageUploader(defaultWebhookUrl = '') {
    if (!imageUploaderModal) return;
    imageUploaderModal.hidden = false;
    imageUploaderModal.setAttribute('aria-hidden', 'false');
    document.body.classList.add('uploader-open');
    uploaderWebhookUrl.value = defaultWebhookUrl || WEBHOOK_URLS[0] || '';
    trapFocus(imageUploaderModal);
  }

  uploaderBackdrop?.addEventListener('click', closeImageUploader);
  uploaderClose?.addEventListener('click', closeImageUploader);
  uploaderCancelBtn?.addEventListener('click', closeImageUploader);
  webhookUploadBtn?.addEventListener('click', () => openImageUploader());

  uploaderFileInput?.addEventListener('change', (e) => {
    const file = e.target.files?.[0];
    if (!file || !file.type.startsWith('image/')) {
      uploaderStatus.textContent = 'Please select a valid image file';
      uploaderStatus.style.color = '#ef4444';
      uploaderSelectedFile = null;
      uploaderSubmitBtn.disabled = true;
      return;
    }

    uploaderSelectedFile = file;
    ...
    const reader = new FileReader();
    reader.onload = (evt) => {
      uploaderPreview.innerHTML = `<img src="${evt.target.result}" alt="Preview" style="max-width: 100%; max-height: 200px; border-radius: 8px;" />`;
      uploaderPreview.style.display = 'block';
    };
    reader.readAsDataURL(file);

    uploaderSubmitBtn.disabled = false;
    console.log('Image selected:', file.name);
  });

  uploaderSubmitBtn?.addEventListener('click', async () => {
    if (!uploaderSelectedFile || !uploaderWebhookUrl.value) {
      uploaderStatus.textContent = 'Please select an image and enter a webhook URL';
      uploaderStatus.style.color = '#ef4444';
      uploaderStatus.className = 'uploader-status error';
      return;
    }

    uploaderSubmitBtn.disabled = true;
    uploaderSubmitBtn.classList.add('loading');
    uploaderStatus.textContent = 'Uploading...';
    uploaderStatus.className = 'uploader-status uploading';

    try {
      const useJsonFormat = uploaderSendAsJson?.checked;
      const useBinaryFormat = uploaderSendBinary?.checked;
      const showDebug = uploaderShowDebug?.checked;
      const useProxy = uploaderUseProxy?.checked;
      const fieldName = (uploaderFieldName?.value?.trim() || 'image').toLowerCase();
      
      console.log('%c📤 UPLOAD START', 'color: #3b82f6; font-weight: bold; font-size: 14px;');
      console.log('URL:', uploaderWebhookUrl.value);
      console.log('File:', uploaderSelectedFile.name, `(${uploaderSelectedFile.size} bytes)`);
      console.log('MIME Type:', uploaderSelectedFile.type);
      console.log('Use Proxy:', useProxy ? '✓ Yes' : '✗ No (direct)');
      
      let uploadFormat = '📦 multipart/form-data';
      if (useJsonFormat) uploadFormat = '🔷 JSON+Base64';
      else if (useBinaryFormat) uploadFormat = '🔲 Raw Binary';
      
      console.log('Format:', uploadFormat);
      console.log('Field name:', fieldName);

      // Determine final upload URL
      const finalUrl = useProxy ? `/api/upload?url=${encodeURIComponent(uploaderWebhookUrl.value)}` : uploaderWebhookUrl.value;

      let response;
      let requestInfo = {};

      if (useBinaryFormat) {
        // Send as raw binary (application/octet-stream)
        const arrayBuffer = await uploaderSelectedFile.arrayBuffer();

        requestInfo.method = 'POST';
        requestInfo.contentType = 'application/octet-stream';
        requestInfo.payloadSize = arrayBuffer.byteLength;
        requestInfo.fields = ['(raw binary data)'];

        console.log('🔲 Raw Binary upload');
        ...
        response = await fetch(finalUrl, {
          ...
          headers: {
            ...
            'X-Filename': uploaderSelectedFile.name,
            'X-Mime-Type': uploaderSelectedFile.type
          },
          body: arrayBuffer
        });
        ...
        const base64Data = await new Promise((resolve, reject) => {
          ...
          reader.readAsDataURL(uploaderSelectedFile);
        });

        const jsonPayload = {
          [fieldName]: base64Data,
          fileName: uploaderSelectedFile.name,
          mimeType: uploaderSelectedFile.type,
          size: uploaderSelectedFile.size
        };

        requestInfo.method = 'POST';
        requestInfo.contentType = 'application/json';
        requestInfo.payloadSize = JSON.stringify(jsonPayload).length;
        requestInfo.fields = Object.keys(jsonPayload);

        console.log('🔷 JSON+Base64 upload');
        console.log('📋 Payload fields:', requestInfo.fields);
        console.log('📊 Payload size:', requestInfo.payloadSize, 'bytes');
        ...
        // Send as multipart/form-data (default)
        const formData = new FormData();
        const fieldNames = fieldName === 'auto' || !fieldName ? ['image', 'file', 'upload', 'attachment'] : [fieldName];

        console.log('📦 multipart/form-data upload');
        console.log('📋 FormData fields to append:', fieldNames);
        
        for (const name of fieldNames) {
          formData.append(name, uploaderSelectedFile, uploaderSelectedFile.name);
          console.log(`  ✓ Appended: ${name} = File(${uploaderSelectedFile.name})`);
        }
        ...
        requestInfo.method = 'POST';
        requestInfo.contentType = 'multipart/form-data (auto)';
        requestInfo.payloadSize = uploaderSelectedFile.size;
        requestInfo.fields = fieldNames;
      ...
      console.log('%c📨 RESPONSE RECEIVED', 'color: #8b5cf6; font-weight: bold; font-size: 14px;');
      ...
      if (showDebug) {
        const debugMsg = `REQUEST:
  Format: ${uploadFormat}
  Method: ${requestInfo.method}
  Content-Type: ${requestInfo.contentType}
  Fields: ${requestInfo.fields.join(', ')}
  Payload: ${requestInfo.payloadSize} bytes

RESPONSE:
  Status: ${response.status} ${response.statusText}
  Body: ${typeof responseData === 'string' ? responseData : JSON.stringify(responseData).substring(0, 200)}`;
        console.log(debugMsg);
        uploaderStatus.innerHTML = `<pre style="font-size: 11px; max-height: 150px; overflow-y: auto; background: var(--muted); padding: 8px; border-radius: 4px; line-height: 1.4;">${DOMPurify.sanitize(debugMsg)}</pre>`;
      }

      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${responseData?.raw || responseData?.message || response.statusText}`);
      }

      console.log('%c✅ UPLOAD SUCCESSFUL', 'color: #10b981; font-weight: bold; font-size: 14px;');
      uploaderStatus.textContent = '✓ Success! Image uploaded.';
      uploaderStatus.className = 'uploader-status success';
      
      setTimeout(() => closeImageUploader(), 2500);
    } catch (error) {
      console.error('%c❌ UPLOAD FAILED', 'color: #ef4444; font-weight: bold; font-size: 14px;', error);
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
        isCorsError = true;
        errorMsg = 'CORS Error: Webhook server blocked the request. This is a server-side configuration issue.';
      }

      uploaderStatus.textContent = `❌ ${errorMsg}`;
      uploaderStatus.style.color = '#ef4444';
      
      if (isCorsError) {
        uploaderStatus.innerHTML += `
<div style="margin-top: 12px; padding: 12px; background: rgba(239, 68, 68, 0.1); border-radius: 6px; font-size: 12px; color: var(--text-2);">
  <strong>Fix: Enable CORS on your webhook server</strong>
  <br>
  <br><strong>For n8n:</strong>
  <ol style="margin: 8px 0; padding-left: 20px;">
    <li>Go to Webhook node settings</li>
    <li>Enable "CORS" in Advanced options</li>
    <li>Set CORS origin to: <code style="background: var(--muted); padding: 2px 4px;">${window.location.origin}</code></li>
  </ol>
</div>`;
      }
      ...
      uploaderStatus.className = 'uploader-status error';
      uploaderSubmitBtn.disabled = false;
      uploaderSubmitBtn.classList.remove('loading');
    }
  });

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && imageUploaderModal && !imageUploaderModal.hidden) closeImageUploader();
  });
  ...
  function addAIMessage(text, images, questions) {
    ...
    // Copy button + Feedback for AI responses
    if (text) {
      const actionsBar = document.createElement('div');
      actionsBar.className = 'response-actions';
      const copyBtn = document.createElement('button');
      copyBtn.className = 'response-action-btn';
      copyBtn.innerHTML = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy';
      copyBtn.addEventListener('click', () => {
        navigator.clipboard.writeText(text).then(() => {
          copyBtn.innerHTML = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg> Copied!';
          setTimeout(() => {
            copyBtn.innerHTML = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy';
          }, 2000);
        });
